...
DT_LabourNeedsPanel = ISPanel:derive("DT_LabourNeedsPanel")
...
local function buildTirednessData(worker)
    if DT_Labour and DT_Labour.Tiredness and isFunction(DT_Labour.Tiredness.GetBarData) then
        return DT_Labour.Tiredness.GetBarData(worker)
    end
...
local function buildNeedsData(worker)
    ...
    return {
        {
            label = "Energy",
            color = DT_Labour and DT_Labour.Tiredness and isFunction(DT_Labour.Tiredness.GetBarColor)
                and DT_Labour.Tiredness.GetBarColor()
...
function DT_LabourNeedsPanel:new(x, y, width, height)
...
function DT_LabourNeedsPanel:initialise()
    ISPanel.initialise(self)
end

function DT_LabourNeedsPanel:setWorkerData(worker)
    if not worker then
        self.subject = nil
        self.loading = false
        return
...
function DT_LabourNeedsPanel:drawNeedRow(entry, x, y, width, height)
...
function DT_LabourNeedsPanel:prerender()
