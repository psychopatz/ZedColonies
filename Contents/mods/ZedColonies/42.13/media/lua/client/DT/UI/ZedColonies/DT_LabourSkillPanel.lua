...
function DT_LabourSkillPanel:new(x, y, width, height)
...
function DT_LabourSkillPanel:initialise()
    ISPanel.initialise(self)
end

function DT_LabourSkillPanel:setWorkerData(worker)
    if not worker then
        self.subject = nil
        self.loading = false
        return
...
function DT_LabourSkillPanel:setPreviewSubject(subject)
    if not subject then
        self.subject = nil
        self.loading = false
        return
    ...
    self.subject = {
        ...
        skills = DT_Labour.Skills.BuildPreviewSkillSnapshot(subject.archetypeID, subject.identitySeed),
        previewOnly = true,
        loading = false
    }
    self.subject.portraitTex = getPortraitTexture(self.subject)
...
function DT_LabourSkillPanel:drawMasteryIcon(x, y)
...
function DT_LabourSkillPanel:drawSkillRow(skill, x, y, width, height)
    local barX = x + 190
...
function DT_LabourSkillPanel:prerender()
