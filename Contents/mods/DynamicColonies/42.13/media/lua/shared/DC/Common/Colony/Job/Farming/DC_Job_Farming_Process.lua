...
function Sim.ProcessFarmJob(worker, ctx)
    local currentHour = ctx.currentHour
    local profile = ctx.profile
    local normalizedJobType = ctx.normalizedJobType
    local speedMultiplier = ctx.speedMultiplier
    ...
    elseif worker.jobEnabled and toolsReady and hasHydration and hasCalories and not forcedRest then
        worker.state = Config.States.Working
        worker.siteState = worker.greenhouseWorkLabel or (Resources and Resources.GetAnyGreenhouseLabel and Resources.GetAnyGreenhouseLabel(worker.ownerUsername)) or "Greenhouse"
        worker.workProgress = Internal.clampHours(worker.workProgress) + (workableHours * speedMultiplier)
        didWorkThisTick = workableHours > 0

        while worker.workProgress >= cycleHours do
            worker.workProgress = worker.workProgress - cycleHours
            local jobResult = Resources and Resources.ProcessFarmWorkCycle and Resources.ProcessFarmWorkCycle(worker, {
                currentHour = currentHour,
                yieldMultiplier = jobSkillEffects and jobSkillEffects.yieldMultiplier or 1
            }) or {
                entries = {},
                totalQuantity = 0,
                success = false,
            ...
            if jobResult.success and (tonumber(jobResult.totalQuantity) or 0) > 0 then
                Internal.logJobCycleOutcome(
                    worker,
                    currentHour,
                    jobResult.totalQuantity,
                    tostring(jobResult.siteLabel or "Greenhouse"),
                    jobResult.entries
                )
                Sim.grantWorkerJobXP(worker, currentHour, jobSkillEffects, jobResult.totalQuantity)
            end

            if warehouseBlocked > 0 then
                Internal.appendWorkerLog(
                    worker,
                    "Warehouse is full. " .. tostring(warehouseBlocked) .. " harvested item" .. (warehouseBlocked == 1 and "" or "s") .. " could not be stored.",
                    currentHour,
                    "warehouse"
                )
                worker.state = Config.States.StorageFull
