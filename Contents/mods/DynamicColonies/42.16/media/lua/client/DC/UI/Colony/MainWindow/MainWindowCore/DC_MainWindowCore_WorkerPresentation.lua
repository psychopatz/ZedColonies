...
function Internal.getJobColor(jobType)
    ...
    if normalizedJob == tostring(jobTypes.Gatherer or "Gatherer") then
        return { r = 0.74, g = 0.86, b = 0.42, a = 1 }
    end
    if normalizedJob == tostring(jobTypes.CorpseRemoval or "CorpseRemoval") then
...
function Internal.getWorkerPresenceLabel(worker)
    ...
    if normalizedJob ~= ((config.JobTypes or {}).Scavenge)
        and normalizedJob ~= ((config.JobTypes or {}).Gatherer) then
