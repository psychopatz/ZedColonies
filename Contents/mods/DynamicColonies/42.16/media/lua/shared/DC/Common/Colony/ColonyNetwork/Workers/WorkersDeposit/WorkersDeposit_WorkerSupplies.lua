...
Network.Handlers.DepositWorkerSupplies = function(player, args)
    ...
    local _, worker = Deposit.getWorkerContext(player, args)
    if not worker then
        return
    end
    ...
    for _, lock in ipairs(reserved) do
        ...
        if invItem then
            ...
            if entry then
                ...
                if DC_Colony.Registry.AddNutritionEntry(worker, entry) then
    ...
    Shared.syncSupplyTransferResult(player, args, {
        ...
        message = Deposit.buildTransferMessage(tostring(FlavorText.npcInventoryLabel or "NPC inventory"), movedCount, #rejected),
    })

    if movedCount <= 0 and eligibleCount > 0 then
        Internal.syncNotice(player, tostring(FlavorText.npcInventoryFull or "NPC inventory is full. No provisions could be deposited."), "error", true)
    ...
    if blockedCount > 0 then
        Internal.syncNotice(
            player,
            string.format(
                tostring(FlavorText.npcInventoryNearlyFull or "NPC inventory is nearly full. %s provision item%s could not be stored."),
